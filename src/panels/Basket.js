...
const Basket = ({ match: { params: { areaId, itemId }}, foodAreas, order }) => {
  ...
  function save_to_ls() {
    ...
    order_info.cafe = item.id;
    order_info.time = time;
    order_info.faster = faster;
    order_info.place_choice = selfService;

    var orders_info = new Map (JSON.parse(localStorage.getItem('orders_info') || 'null'))
    orders_info.set(order_info.cafe, order_info)
    localStorage.setItem('orders_info',  JSON.stringify([...orders_info]));
  }
  ...
  useEffect(() => {
    var orders_info = new Map (JSON.parse(localStorage.getItem('orders_info') || 'null'))
    var order_info = orders_info.get(item.id);
    if (order_info != undefined) {
      setTime(order_info.time);
      setFaster(order_info.faster);
      setSelfService(order_info.place_choice);
    }
  ...
  return (
    <div className="Place">
      ...
      <footer className="Place__footer">
        <Link to={`/order/${area.id}/${item.id}`} className="Place__order" onClick={save_to_ls}>
          Оплатить {price}
        </Link>
      </footer>
    </div>
